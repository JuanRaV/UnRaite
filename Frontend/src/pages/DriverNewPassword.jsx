...
const DriverNewPassword = () => {
    ...
    useEffect(()=>{
        const checkToken = async()=>{
            try {
                await axiosClient.get(`/api/users/forgot-password/driver/${token}`)
                setValidToken(true)
            } catch (error) {
                setAlert({
    ...
    const handleSubmit = async e=>{
        ...
        try {
            const {data} = await axiosClient.post( `/api/users/forgot-password/driver/${token}`,{password})
            setPassword("")
            setAlert({
                msg:data.msg,
                error:false
...
export default DriverNewPassword